...
def get_propensity(rates: np.ndarray, curH: int, curI: int) -> np.ndarray:
    ...
    prop = np.zeros(6, dtype=np.float64)
    prop[0] = r1 * curH
    prop[1] = r2 * curH
    prop[2] = b1 * curI
...
def tau_twocomp_carrier(
    ...
    """Simulate the two-comp model.

    Parameters
    ----------
    init_load : array_like
    ...
    status
        ...
        3 : Succesful completion, terminated when I(t) > i_thresh.
    ...
    while ind < nstep:
        ...
        mu[0] = np.abs(-1.0 * prop_array[0] * J[0] + -1.0 * prop_array[1] * J[1])
        mu[1] = np.abs(
        ...
        sigsq[0] = 1.0 * prop_array[0] * J[0] + 1.0 * prop_array[1] * J[1]
        sigsq[1] = (
            1.0 * prop_array[2] * J[2]
            + 1.0 * prop_array[3] * J[3]
            + 1.0 * prop_array[4] * J[4]
